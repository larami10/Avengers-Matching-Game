...
function appendNewCardTest() {
  ...
  if (parent.children.length !== 1) {
    console.error(
      `${ERROR} This function should be appending a new element as a child of the 'parentElement' parameter but the number of children on 'parentElement' is ${parent.children.length}.`
    );
    console.log(
  ...
  if (!card.classList.contains("card")) {
    console.error(
      `${ERROR} This function should be adding a card div with the class 'card' as the first child of 'parentElement' but the child of 'parentElement' doesn't have the 'card' class.`
    );
    console.log(
      "\nHere's what the HTML looks like:\n\n" + serialize(parent) + "\n\n"
    );
  ...
  if (card.children.length !== 2) {
    console.error(
      `${ERROR} card div should have 2 children, got ${card.children.length}. The card-container should have a div with the class card-down and another div with the class card-up.`
    );
    return false;
  }
  ...
  if (!cardDown.classList.contains("card-down")) {
    console.error(
      `${ERROR} card div's first child should have 'card-down' class, got ${cardDown.classList}. Make sure to add the card-down class to the first div inside card.`
    );
    return false;
  }
  ...
  if (!cardUp.classList.contains("card-up")) {
    console.error(
      `${ERROR} card div's second child should have 'card-up' class, got ${cardUp.classList}. Make sure to add the card-up class to the second div inside card.`
    );
    return false;
  }
  ...
  if (returnValue !== card) {
    console.error(
      `${ERROR} Remember to return the card! The return value of this function should be the card.`
    );
    return false;
  }
...
function shuffleCardImageClassesTest() {
  ...
  if (typeof imageArray1 !== "object") {
    console.error(
      `${ERROR}should return an array of strings but returned ${typeof imageArray1}. Make sure this function is defining and returning an array.`
    );
    return false;
  }
  ...
  if (imageArray1.length !== 12) {
    console.error(
      `${ERROR}should return an array of 12 strings but got ${imageArray1.length}.`
    );
    return false;
  }
  ...
  for (let i = 0; i < imageArray1.length; i++) {
    if (
      !imageArray1[i].startsWith("image-") ||
  ...
  if (!imageArray1.every((element) => typeof element === "string")) {
    console.error(
      `${ERROR}should return array of all strings but array elements were not all strings. Make sure all elements are in fact all strings.`
    );
    return false;
  }
  ...
  if (allSame) {
    console.error(
      `${ERROR}should return random array but 4 comparison calls returned the same array. Make sure to shuffle!`
    );
    return false;
  }
  ...
  if (badName != "") {
    console.error(
      `${ERROR}should return array where names start with 'image-X' (1-6) but got '${badName}'.`
    );
    return false;
  }
...
function createCardsTest() {
  ...
  if (parent.children.length != 12) {
    console.error(
      `${ERROR}should create 12 card objects as children but got ${parent.children.length}. Make sure you are adding the right number of cards to the card-grid-container!`
    );
    return false;
  }
  ...
  if (typeof cards !== "object" || cards.length == "undefined") {
    console.error(
      `${ERROR}should return an array of objects but returned ${typeof cards}.`
    );
    return false;
  }
  ...
  if (cards.length !== 12) {
    console.error(
      `${ERROR}should return 12 card objects as children but got ${parent.children.length}. Make sure to push 12 cards onto the cards array!`
    );
    return false;
  }
  ...
  if (badObject !== null) {
    console.error(
      `${ERROR}card objects should each have index, element, and imageClass properties but got...`
    );
...
function incrementCounterTest() {
  ...
  if (!string1) {
    console.error(
      `${ERROR}parentElement has not been updated, make sure to change it's innerHTML.`
    );
    return false;
  }
  ...
  if (value1 !== 1) {
    console.error(
      `${ERROR}should place '1' in parent (counter should increase to 1) after first call but got '${string1}'`
    );
    return false;
  }
  ...
  if (value2 - value1 !== 1) {
    console.error(
      `${ERROR}should increment value in parent by 1 but consecutive calls to the same counter but got '${string1}' and '${string2}'`
    );
    return false;
  }
  ...
  if (value3 !== 1) {
    console.error(
      `${ERROR}should place '1' in parent after first call with new counterName but got '${string3}'`
    );
    return false;
  }
